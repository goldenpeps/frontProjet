...
import { InterventionCard, InterventionTableRowCells } from './InterventionsDisplay';
import {
...
function InterventionsContent() {
  const [formData, setFormData] = useState<InterventionFormData>(emptyInterventionForm);
  const [showModal, setShowModal] = useState(false);
  const [editingIntervention, setEditingIntervention] = useState<Intervention | null>(null);
  const [formError, setFormError] = useState('');
  const [formLoading, setFormLoading] = useState(false);
  const [confirmIntervention, setConfirmIntervention] = useState<Intervention | null>(null);
  ...
  const handleEdit = (intervention: Intervention) => {
    ...
    setFormData({
      ...
      heure_prevue: planningMeta?.heurePrevue ?? extractTimePart(intervention.date_prevue),
      ...
      materiel_utilise_id: intervention.materiel_utilise_id == null ? '' : String(intervention.materiel_utilise_id),
      materiel_ids: linkedMaterielUtilise ? linkedMaterielUtilise.materiels.map((m) => String(m.id)) : [],
      equipe_intervention_id: intervention.equipe_intervention_id == null ? '' : String(intervention.equipe_intervention_id),
      client_id: planningMeta?.clientId == null ? '' : String(planningMeta.clientId),
  ...
  const handleSubmit = async () => {
    setFormError('');
    if (!formData.date_prevue || !formData.date_realisation || !formData.client_id || !formData.terrain_id) {
      setFormError('Les dates début/fin, le client et le terrain sont obligatoires');
      return;
    }
    ...
    if (endDate <= startDate) {
      setFormError('La date/heure de fin doit être strictement après la date/heure de début');
      return;
    }
    ...
    const heurePrevue = extractTimePart(formData.date_prevue) || formData.heure_prevue;

    if (formData.type_intervention === 'tonte') {
      const gpsCoordinates = extractGpsCoordinates(terrain.coordonnees_gps);
      if (!gpsCoordinates) {
        setFormError('Ce terrain ne contient pas de coordonnees GPS exploitables pour verifier la meteo');
        return;
      }
      ...
      if (lastMowingDate) {
        ...
        if (deltaDays < MIN_DAYS_BETWEEN_MOWING) {
          setFormError(
            `Tonte impossible: la derniere tonte date du ${lastMowingDate.toLocaleDateString('fr-FR')} (${deltaDays} jours). Minimum ${MIN_DAYS_BETWEEN_MOWING} jours.`
          );
          return;
        }
      }
      ...
      try {
        ...
        if (weather.hasRainOrSnow) {
          const { name: terrainName, address: terrainAddress } = getTerrainNameAndAddress(terrain);
          const acceptRainRisk = await showAlertifyConfirmation({
            title: 'Alerte meteo - risque pluie/neige',
            weatherDescription: weather.description,
            dateLabel: new Date(formData.date_prevue).toLocaleDateString('fr-FR'),
            terrainName,
            terrainAddress,
            gps: gpsCoordinates,
          ...
          if (!acceptRainRisk) {
            setFormError('Intervention annulee: tonte refusee en raison du risque de pluie');
            return;
          }
        }
      } catch (weatherError) {
        const weatherMessage = weatherError instanceof Error ? weatherError.message : '';
        const noForecastData = /donnees meteo indisponibles|impossible de recuperer la meteo/i.test(weatherMessage);

        if (noForecastData) {
          const { name: terrainName, address: terrainAddress } = getTerrainNameAndAddress(terrain);
          const continueWithoutForecast = await showAlertifyConfirmation({
            title: 'Meteo indisponible',
            weatherDescription: "L'API meteo n'a pas encore de prevision pour cette date",
            dateLabel: new Date(formData.date_prevue).toLocaleDateString('fr-FR'),
            terrainName,
            terrainAddress,
            gps: gpsCoordinates,
          ...
          if (!continueWithoutForecast) {
            setFormError('Intervention annulee: verification meteo indisponible pour cette date');
            return;
          }
        } else {
          setFormError(weatherMessage || 'Erreur lors de la verification meteo');
          return;
        }
      }
    }
    ...
    try {
      ...
      const payload = {
        ...
        equipe_intervention_id: toNullableNumber(formData.equipe_intervention_id),
      };
      ...
      if (editingIntervention) {
        ...
        if (formData.type_intervention === 'tonte' || formData.type_intervention === 'ramassage') {
          await historiqueTerrainService.create({
            ...
            dateRamassage: formData.type_intervention === 'ramassage' ? historiqueDate : null,
            dateTonte: formData.type_intervention === 'tonte' ? historiqueDate : null,
          });
        }
  ...
  return (
    <>
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <div>
            ...
            <p className={styles.subtitle}>{interventions.length} intervention{interventions.length > 1 ? 's' : ''}</p>
          </div>
      ...
      {showModal && (
        <div className={styles.overlay} onClick={() => setShowModal(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <h2 className={styles.modalTitle}>{editingIntervention ? 'Modifier l\'intervention' : 'Nouvelle intervention'}</h2>
            ...
            <div className={styles.formGrid}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  onChange={(e) => setFormData((prev) => ({ ...prev, heure_prevue: e.target.value }))}
                />
              </div>
              <div className={styles.formGroup}>
              ...
              <div className={styles.formGroup}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const nextType = e.target.value === 'tonte'
                      ? 'tonte'
                      : e.target.value === 'ramassage'
                        ? 'ramassage'
                        : 'autre';
                    setFormData((prev) => ({ ...prev, type_intervention: nextType }));
                  }}
                >
              ...
              <div className={styles.formGroup}>
                ...
                <select
                  ...
                  onChange={(e) => setFormData((prev) => ({ ...prev, terrain_id: e.target.value }))}
                >
                  <option value="">Selectionner un terrain</option>
                  {filteredTerrains.map((terrain) => (
                    <option key={terrain.id} value={terrain.id}>
                      {formatTerrainLabel(terrain, clientById.get(terrain.client_id))}
                    </option>
                  ))}
                </select>
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Équipe d'intervention</label>
                <select
                  className={styles.formInput}
                  value={formData.equipe_intervention_id}
                  onChange={(e) => setFormData((prev) => ({ ...prev, equipe_intervention_id: e.target.value }))}
                >
              ...
              <div className={styles.formGroup + ' ' + styles.formGroupFull}>
                <label className={styles.formLabel}>Matériels utilisés (1 à plusieurs) *</label>
                <select
                  className={styles.formInput}
                  multiple
                ...
                >
                  {materiels.map((materiel) => (
                    <option key={materiel.id} value={String(materiel.id)}>
                      #{materiel.id} - {materiel.type_materiel_libelle || `Type #${materiel.type_materiel_id ?? '—'}`} ({materiel.disponible ? 'disponible' : 'indisponible'})
                    </option>
                  ))}
                </select>
                <p className={styles.formHint}>Maintenez Ctrl (Windows) pour sélectionner plusieurs matériels.</p>
              </div>
              <div className={styles.formGroup + ' ' + styles.formGroupFull}>
            ...
            <div className={styles.modalActions}>
              ...
              <button className={styles.btnSave} onClick={handleSubmit} disabled={formLoading}>
                {formLoading ? 'Enregistrement...' : editingIntervention ? 'Enregistrer' : 'Créer'}
              </button>
            </div>
          </div>
        </div>
      ...
      {confirmIntervention && (
        <div className={styles.overlay} onClick={() => setConfirmIntervention(null)}>
          <div className={styles.modal + ' ' + styles.modalSmall} onClick={(e) => e.stopPropagation()}>
            <h2 className={styles.modalTitle}>Confirmation</h2>
            <p className={styles.confirmText}>Supprimer l'intervention #{confirmIntervention.id} ?</p>
            <div className={styles.modalActions}>
              <button className={styles.btnCancel} onClick={() => setConfirmIntervention(null)}>
                Annuler
              </button>
              <button className={styles.btnDeactivate} onClick={() => handleDelete(confirmIntervention)}>
                Supprimer
              </button>
            </div>
          </div>
